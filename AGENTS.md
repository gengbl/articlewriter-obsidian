...
| `src/main.ts` | 插件入口：设置、命令注册（`addCommand`）、每个命令一个 `cmdXxx()` handler、通用交互辅助方法 |
...
- 删除一律走回收站：`vault.trash(file, false)`（本地 `.trash/`），不用硬删；角色改名会把被改动文件备份到 `_backup/` 后再全小说替换。
