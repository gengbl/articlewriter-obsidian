...
| 写字台 | —（插件新增） | **常驻可停靠面板**（侧边栏书本图标快捷入口）：工作目录、全部小说列表（点击切换当前书）、当前小说的题材/编写类型/总字数/更新时间、章节列表（点击激活该章并同步所属卷）、全局文档与各章文件（点击在编辑器打开），小说/章节/文件列表均支持像文件夹一样展开折叠（点标题行或章节前的箭头）；**右键**小说/章节/文件行有快捷菜单：新建或删除小说、新建或删除章节、在书根目录或章节目录下新建文章 .md / 删除文件（危险操作均有二次确认，删除进 Obsidian 回收站可找回），右上「刷新」手动重载数据 |
