...
## 卷ID | 名称 | 描述
例：## v01 | 第一卷 风起 | 主角入世
每个卷在书根下拥有同名实体目录 <书名>/<卷名>/，归属该卷的章节目录存放其中；未归属章节留在书根。章号按容器独立编号（各卷内从第01章重新起算、互不冲突），跨卷引用须写明卷名（如「第二卷·第5章」）。卷名须唯一（作为目录名）且不得含冒号。
-->
`;

/** 卷级大纲模板（v0.0.15：卷目录下 卷大纲.md，写作时注入当前卷上下文） */
export const VOL_OUTLINE_TEMPLATE = (volName: string): string =>
	`# ${volName} 卷大纲\n\n` +
	"<!-- 示例（按需填充；本卷整体走向会注入 /write /continue /review 等提示词的卷级上下文）：\n" +
	"- 本卷主线：……\n" +
	"- 关键节点：\n" +
	"  - ……\n" +
	"- 收束目标：……\n" +
...
/** 卷级人物模板（卷目录下 人物.md，与全局/章节级人物合并进入提示词） */
export const VOL_CHARACTERS_TEMPLATE = `# 人物

<!--
归属本卷的角色设定（卷内通用角色放这里，单章专属角色仍放各章节目录的 人物.md）。格式：每行一个字段
## 角色名
- 身份：
- 性别：
- 性格：
...
/** 卷级场景模板（卷目录下 场景.md） */
export const VOL_SCENES_TEMPLATE = (volName: string): string =>
	`# ${volName} 场景\n\n` +
	"<!--\n" +
	"本卷出现的场景（跨多章复用的地点放这里；单章场景仍放各章节目录的 场景.md）。示例：\n" +
	"## scene_01\n" +
	"- 描述：……\n" +
	"-->\n";

/** 《时间线》文档模板（书根 / 卷目录 / 章节目录三层播种同一来源，H1 由调用方拼好传入；注释内为用法示例——面板解析经 stripComments 过滤、不参与展示） */
export const timelineTemplate = (h1Title: string): string =>
	`# ${(h1Title || "时间线").trim()}\n\n` +
	"<!--\n" +
	"记录本层发生的事件。「时间线」面板会把书/卷/章三层的《时间线.md》合并成一条按时间点升序排列的纵向时间轴。\n" +
	"\n" +
	"写法：每条事件一个块，标题＝时间点（支持 `年`、`年-月`、`年-月-日`，如 120 / 120-03 / -129-06-15），块内字段行记人物与事件。示例：\n" +
	"## -129-06-15\n" +
	"- 人物：林某、赵某\n" +
	"- 事件：示例事件——某年某月某日发生了什么。\n" +
	"\n" +
	"非有效时间点的标题块整块忽略；删除或改写本注释不影响解析。\n" +
	"-->\n";
