...
| Timeline panel | **Persistent dockable panel** (history icon in the sidebar as quick entry): renders the current story's three tiers of optional hand-written 《时间线.md》 docs (book root / volume dir / chapter dirs) as a top-to-bottom vertical timeline — volume mode shows book level → unassigned chapters → each volume with its member chapters nested inside; flat mode shows book level + all chapters; entries within each source group sort ascending by time-point number (negatives/decimals allowed). A filter box on top matches character/event keywords (groups without hits fold away), clicking a group header collapses/expands it (kept for the session), and clicking an entry opens that tier's source md. Timeline docs are opt-in reference material only (never injected into writing prompts): place them at `<book>/时间线.md`, `<volDir>/<卷名>-时间线.md` (bare `时间线.md` also works) or `<chDir>/NN-<标题>-时间线.md` (bare name also works); format = one time point per `## <number>` heading, `- 人物：A、B` field line for involved characters and `- 事件：<text>` for the event (free text or ```text fenced blocks may follow), non-numeric headings skip their whole block; edits to the docs auto-refresh the panel |
